...
configuration = plaid.Configuration(
    host=host, api_key={"clientId": PLAID_CLIENT_ID, "secret": PLAID_SECRET, "plaidVersion": "2020-09-14"}
)
...
def info():
    global access_token
    global item_id
    return jsonify({"item_id": item_id, "access_token": access_token, "products": PLAID_PRODUCTS})
...
def get_identity():
    try:
        ...
        pretty_print_response(response.to_dict())
        return jsonify({"error": None, "identity": response.to_dict()["accounts"]})
    except plaid.ApiException as e:
        error_response = format_error(e)
        return jsonify(error_response)
        ...
        request = TransferAuthorizationCreateRequest(
            ...
            user=TransferAuthorizationUserInRequest(
                ...
                address=TransferUserAddressInRequest(
                    street="123 Main St.", city="San Francisco", region="CA", postal_code="94053", country="US"
                ),
            ),
        )
...
def transfer():
    try:
        request = TransferCreateRequest(
            access_token=access_token, account_id=account_id, authorization_id=authorization_id, description="Debit"
        )
